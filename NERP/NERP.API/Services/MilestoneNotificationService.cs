    ...
    public async Task ProcessMilestonesAsync()
    {
        ...
        foreach (var employee in employees)
        {
            ...
            if (employee.BirthDate.HasValue &&
                employee.BirthDate.Value.Month == today.Month &&
                employee.BirthDate.Value.Day == today.Day)
            {
                birthdayCount++;
                ...
                employee.JoiningDate.Value.Day == today.Day)
            {
                ...
                // Per HR policy, Work Anniversary is only a named milestone
                // at the 3, 5, and 10 year marks — not every single year.
                ...
                {
                    _logger.LogInformation("Work anniversary skipped for {Name} — {Years} years completed is not a 3/5/10-year milestone", employee.Name, years);
                }
            }
        }
