...
const UserAvatar = React.forwardRef<
  React.ElementRef<typeof AvatarPrimitive.Root>,
  UserAvatarProps
>(({ name, size = "h-10 w-10", className, fallbackClassName, ...props }, ref) => {
  const initials = getInitials(name);
  const colorClass = getAvatarColorClass(name);
  return (
    <Avatar ref={ref} className={cn(size, className)} {...props}>
      <AvatarFallback className={cn("text-xs font-semibold uppercase shadow-inner border border-white/10", colorClass, fallbackClassName)}>
        {initials}
      </AvatarFallback>
    </Avatar>
  );
