...
export default function MyPointsPanel() {
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {(user?.selfDevelopmentAccumulatedPoints ?? 0) > 0 && (
        <div className="rounded-2xl border border-blue-100 bg-blue-50/60 p-4 dark:border-blue-900/40 dark:bg-blue-950/20">
          <div className="flex items-center justify-between text-sm">
            <span className="font-semibold text-slate-800 dark:text-slate-200">📚 Self Development Progress</span>
            <span className="font-semibold text-blue-700 dark:text-sky-400">
              {user?.selfDevelopmentAccumulatedPoints}/500 pts
            </span>
          </div>
          <div className="mt-2 h-2 w-full overflow-hidden rounded-full bg-white dark:bg-slate-900">
            <div
              className="h-full rounded-full bg-blue-600 transition-all"
              style={{ width: `${Math.min(100, ((user?.selfDevelopmentAccumulatedPoints ?? 0) / 500) * 100)}%` }}
            />
          </div>
          <p className="mt-1.5 text-xs text-slate-500 dark:text-slate-400">
            Training points accumulate 100 at a time and are credited to your balance once they reach 500.
          </p>
        </div>
      )}
