      ...
      <CardContent className="space-y-1">
        {isLoading ? (
          ...
          items.map((item) => (
            ...
            >
              <UserAvatar name={item.fromEmployee.name} avatar={item.fromEmployee.avatar} size="h-8 w-8" fallbackClassName="text-xs font-bold" />
              <ArrowRight className="h-3.5 w-3.5 flex-shrink-0 text-muted-foreground" />
              <UserAvatar name={item.toEmployee.name} avatar={item.toEmployee.avatar} size="h-8 w-8" fallbackClassName="text-xs font-bold" />
