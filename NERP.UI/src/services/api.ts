...
export interface ApiCurrentUser {
  ...
  // totalPoints — see the Points page for the progress bar this drives.
  selfDevelopmentAccumulatedPoints?: number;
}
...
export interface ApiAwardCategory {
  ...
  icon: string;
  managerOnly: boolean;
  // 0 = instant, no approval (Kudos). 1 = single Admin/HR approval (Spot
  // Awards). 2 = a peer manager reviews first, then Admin/HR gives final
  // approval (Employee of the Quarter, Rising Star).
  approvalLevel: number;
  // Self Development only: points accrue in increments and are only
  // credited to the wallet once they cross accumulationThreshold.
  isAccumulative: boolean;
  accumulationIncrement: number;
  accumulationThreshold: number;
  // Rising Star only: nominee must have joined within recentJoinerMaxMonths.
  requiresRecentJoiner: boolean;
  recentJoinerMaxMonths: number;
}
...
export interface ApiRecognition {
  ...
  status: "approved" | "pending_peer_review" | "pending" | "rejected";
  likeCount?: number;
  commentCount?: number;
  likedByMe?: boolean;
}
...
export const recognitionsApi = {
  ...
  // ---- Peer review (2-level categories: Employee of the Quarter, Rising Star) ----
  // Nominations awaiting a peer manager's review (never the nominator's own).
  getPeerReviewQueue: () => apiFetch<ApiRecognition[]>("/recognitions/peer-review-queue"),
  peerApprove: (id: number) => apiFetch<{ success: boolean }>(`/recognitions/${id}/peer-approve`, { method: "PUT" }),
  peerReject: (id: number) => apiFetch<{ success: boolean }>(`/recognitions/${id}/peer-reject`, { method: "PUT" }),
...
export const milestonesApi = {
  ...
  getForEmployee: (employeeId: number) => apiFetch<ApiMilestone[]>(`/milestones/${employeeId}`),
};
